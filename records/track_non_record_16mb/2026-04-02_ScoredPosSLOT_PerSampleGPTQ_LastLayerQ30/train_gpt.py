...
try:
    ...
    brotli = None  # installed lazily after dist.init
import math
import os
import random
import subprocess
...
from adaptive_last_layer_qgain import attach_adaptive_last_layer_qgain
try:
...
class Hyperparameters:
    ...
    last_layer_qgain_enabled = bool(int(os.environ.get("LAST_LAYER_QGAIN_ENABLED", "1")))
    last_layer_qk_gain_init = float(os.environ.get("LAST_LAYER_QK_GAIN_INIT", 3.0))
    adaptive_last_layer_qgain_enabled = bool(int(os.environ.get("ADAPTIVE_LAST_LAYER_QGAIN_ENABLED", "0")))
    adaptive_last_layer_qgain_head_groups = int(os.environ.get("ADAPTIVE_LAST_LAYER_QGAIN_HEAD_GROUPS", 4))
    adaptive_last_layer_qgain_log_delta_clamp = float(os.environ.get("ADAPTIVE_LAST_LAYER_QGAIN_LOG_DELTA_CLAMP", 1.0))
    adaptive_last_layer_qgain_effective_max = float(os.environ.get("ADAPTIVE_LAST_LAYER_QGAIN_EFFECTIVE_MAX", 3.0))
...
def apply_last_layer_qgain_override(
    ...
    if not enabled:
        return
    if len(blocks) == 0:
        raise ValueError("Expected at least one transformer block for last-layer q_gain override.")
    with torch.no_grad():
        blocks[-1].attn.q_gain.fill_(qk_gain_init)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gate,skip_gates,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        adaptive_last_layer_qgain = getattr(self, "adaptive_last_layer_qgain", None)
        if adaptive_last_layer_qgain is not None:
            q_gain = (q_gain * adaptive_last_layer_qgain.per_head_multiplier(q_gain)).to(dtype=q.dtype)
        q = q * q_gain[None, None, :, None]
        if HAS_FA3:
            y = flash_attn_3_func(q, k, v, causal=True)
        else:
            y = F.scaled_dot_product_attention(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), is_causal=True, enable_gqa=True).transpose(1, 2)
        if self.use_xsa:
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        last_layer_qgain_enabled: bool = False,
        last_layer_qk_gain_init: float = 3.0,
        adaptive_last_layer_qgain_enabled: bool = False,
        adaptive_last_layer_qgain_head_groups: int = 4,
        adaptive_last_layer_qgain_log_delta_clamp: float = 1.0,
        adaptive_last_layer_qgain_effective_max: float = 3.0,
        mtp_num_heads: int = 0,
        ...
        apply_last_layer_qgain_override(
            self.blocks,
            enabled=last_layer_qgain_enabled,
            qk_gain_init=last_layer_qk_gain_init,
        )
        attach_adaptive_last_layer_qgain(
            self.blocks,
            enabled=adaptive_last_layer_qgain_enabled,
            num_heads=num_heads,
            head_groups=adaptive_last_layer_qgain_head_groups,
            log_delta_clamp=adaptive_last_layer_qgain_log_delta_clamp,
            effective_q_gain_max=adaptive_last_layer_qgain_effective_max,
        )
        if rope_dims > 0:
            head_dim = model_dim // num_heads
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return hidden states (bsz, seq_len, dim) before logit projection."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
    ...
    def compute_logits(self, hidden: Tensor) -> Tensor:
        """Compute logits from hidden states. Supports grad flow for SLOT delta."""
        if self.tie_embeddings:
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 last_layer_qgain_enabled=False, last_layer_qk_gain_init=3.0,
                 adaptive_last_layer_qgain_enabled=False,
                 adaptive_last_layer_qgain_head_groups=4,
                 adaptive_last_layer_qgain_log_delta_clamp=1.0,
                 adaptive_last_layer_qgain_effective_max=3.0,
...
def main() -> None:
    ...
    # Ensure brotli is available (rank 0 installs, others wait via barrier)
    global brotli
    if brotli is None:
        if rank == 0:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", "brotli"])
        if distributed:
            dist.barrier()
        import brotli
    ...
    base_model = GPT(
        ...
        last_layer_qgain_enabled=args.last_layer_qgain_enabled,
        last_layer_qk_gain_init=args.last_layer_qk_gain_init,
        adaptive_last_layer_qgain_enabled=args.adaptive_last_layer_qgain_enabled,
        adaptive_last_layer_qgain_head_groups=args.adaptive_last_layer_qgain_head_groups,
        adaptive_last_layer_qgain_log_delta_clamp=args.adaptive_last_layer_qgain_log_delta_clamp,
        adaptive_last_layer_qgain_effective_max=args.adaptive_last_layer_qgain_effective_max,
    ...
    log0(
        f"q_gain:global_init:{args.qk_gain_init} "
        f"last_layer_enabled:{int(args.last_layer_qgain_enabled)} "
        f"last_layer_init:{args.last_layer_qk_gain_init}"
    )
    log0(
        f"adaptive_last_layer_qgain:enabled:{int(args.adaptive_last_layer_qgain_enabled)} "
        f"head_groups:{args.adaptive_last_layer_qgain_head_groups} "
        f"log_delta_clamp:{args.adaptive_last_layer_qgain_log_delta_clamp} "
        f"effective_max:{args.adaptive_last_layer_qgain_effective_max}"
    )
    ...
    # Use real training data for GPTQ calibration (better Hessians than AR self-gen)
    log0(f"gptq:collecting hessians from training data ({args.gptq_calib_batches} batches)...")
    calib_loader = DistributedTokenLoader(args.train_files, 0, 1, device)
    hessians = collect_hessians(hessian_model, calib_loader, args, device, 1, num_batches=args.gptq_calib_batches)
    ...
    def _byte_shuffle(data: bytes, stride: int = 2) -> bytes:
        """Interleave bytes at stride for better compression (groups similar bytes)."""
        n = len(data)
        out = bytearray(n)
        idx = 0
        for s in range(stride):
            for i in range(s, n, stride):
                ...
                idx += 1
        return bytes(out)
    def _byte_unshuffle(data: bytes, stride: int = 2) -> bytes:
    ...
    if ones_info:
        ...
        def _try_prune(n):
            ...
            return len(brotli.compress(_byte_shuffle(buf.getvalue()), quality=11)) + code_bytes_est, tmp
    ...
    quant_state = torch.load(
        io.BytesIO(_byte_unshuffle(brotli.decompress(quant_blob_disk))),
        map_location="cpu",
        weights_only=False,
    )
