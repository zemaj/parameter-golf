...
class Hyperparameters:
    ...
    tokenizer_meta_path = os.environ.get("TOKENIZER_META_PATH", str(Path(__file__).with_name("candidate.meta.npz")))
    tokenizer_meta_validate = bool(int(os.environ.get("TOKENIZER_META_VALIDATE", "0")))
    ngram_cache_enabled = bool(int(os.environ.get("NGRAM_CACHE_ENABLED", "1")))
...
class NgramBackoffCache:
    """Exact multi-order backoff cache with incremental causal updates."""
    ...
    def __init__(
        ...
        *,
        ...
        self.counts = {order: {} for order in range(2, max_order + 1)}
        self.totals = {order: {} for order in range(2, max_order + 1)}
    ...
    def update_incremental(self, new_tokens: list[int]) -> None:
        for token in new_tokens:
            ...
            for order in range(2, self.max_order + 1):
                ...
                if pos < ctx_len:
                    continue
                context_key = tuple(self.history[pos - ctx_len:pos]) if order > 2 else self.history[pos - 1]
                counts = self.counts[order]
                totals = self.totals[order]
                ctx_counts = counts.setdefault(context_key, {})
                ctx_counts[token] = ctx_counts.get(token, 0) + 1
                totals[context_key] = totals.get(context_key, 0) + 1

    def query_backoff(self, context: list[int], target: int) -> float | None:
        for order in range(self.max_order, 1, -1):
            ctx_len = order - 1
            if len(context) < ctx_len:
                continue
            context_key = tuple(context[-ctx_len:]) if order > 2 else context[-1]
            if context_key not in self.counts[order]:
                continue
            counts_ctx = self.counts[order][context_key]
            total = self.totals[order][context_key]
            count = counts_ctx.get(target, 0)
    ...
    def mix_logprobs(
        self,
        model_logprobs: Tensor,
        model_entropy: Tensor,
        context_tokens: list[int],
        targets: list[int],
    ) -> Tensor:
        mixed = model_logprobs.clone()
        num_targets = len(targets)
        for idx in range(num_targets):
            ctx_end = len(context_tokens) - num_targets + idx
            ctx_start = max(0, ctx_end - (self.max_order - 1))
            ctx = context_tokens[ctx_start:ctx_end]
            ngram_logprob = self.query_backoff(ctx, targets[idx])
            if ngram_logprob is None:
                continue
            alpha = self.alpha_base + self.alpha_range * self._sigmoid(
                2.0 * (float(model_entropy[idx]) - self.entropy_center)
            )
            model_lp = float(mixed[idx])
            max_lp = max(model_lp + math.log(1.0 - alpha), ngram_logprob + math.log(alpha))
            mixed[idx] = max_lp + math.log(
                math.exp(model_lp + math.log(1.0 - alpha) - max_lp)
                + math.exp(ngram_logprob + math.log(alpha) - max_lp)
            )
        return mixed
...
def eval_val_sliding(
    ...
    batch_seqs: int = 32,
    eval_seq_len: int | None = None,
    ngram_cache: NgramBackoffCache | None = None,
) -> tuple[float, float]:
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            if ngram_cache is not None:
                log_probs_all = F.log_softmax(logits_flat, dim=-1)
                probs_all = log_probs_all.exp()
                entropy_all = -(probs_all * log_probs_all).sum(dim=-1).reshape(bsz, seq_len)
            for i, ws in enumerate(batch_ws):
                wlen = wlens[i]
                s = 0 if ws == 0 else max(wlen - stride, 0)
                scored_nll = nll[i, s:wlen].to(torch.float64)
                if ngram_cache is not None:
                    model_logprobs = -scored_nll
                    model_entropy = entropy_all[i, s:wlen].to(torch.float64)
                    prefix_end = ws + wlen + 1
                    prefix_tokens = val_tokens[:prefix_end].tolist()
                    targets = y_batch[i, s:wlen].tolist()
                    mixed_logprobs = ngram_cache.mix_logprobs(
                        model_logprobs,
                        model_entropy,
                        prefix_tokens,
                        targets,
                    )
                    scored_nll = -mixed_logprobs
                ...
                byte_count += tb.sum()
                if ngram_cache is not None:
                    newly_scored_tokens = y_batch[i, s:wlen].tolist()
                    ngram_cache.update_incremental(newly_scored_tokens)
    if ngram_cache is None and dist.is_available() and dist.is_initialized():
...
def main() -> None:
    ...
    log0(
        f"ngram_cache:enabled={args.ngram_cache_enabled} order={args.ngram_order} "
        f"alpha={args.ngram_alpha_base}+{args.ngram_alpha_range}*sigmoid(H-{args.ngram_entropy_center})"
    )
