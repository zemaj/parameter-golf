...
class GPT(nn.Module):
    ...
    def forward_hidden(self, input_ids: Tensor) -> Tensor:
        """Return final hidden states before the output projection."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
    ...
    def compute_logits(self, hidden_states: Tensor) -> Tensor:
        """Project hidden states to logits with the usual output softcap."""
        if self.tie_embeddings:
...
def main() -> None:
    ...
    if args.slot_enabled:
        ...
        ws_list = [ws for ws in range(0, total_tok, slot_stride)
        ...
        for bi in range(0, len(my_ws), 32):
            ...
            with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                hidden_states = eval_model.forward_hidden(xb)
            ...
            slot_opt = torch.optim.AdamW([delta], lr=args.slot_lr, weight_decay=1e-8, eps=1e-5)
            for _ in range(args.slot_steps):
                slot_opt.zero_grad()
                slot_logits = eval_model.compute_logits((hidden_states + delta).to(torch.bfloat16)).float()
                ...
                slot_opt.step()
            with torch.no_grad():
                slot_logits = eval_model.compute_logits((hidden_states + delta.detach()).to(torch.bfloat16)).float()
            slot_nll = F.cross_entropy(
                slot_logits.reshape(-1, slot_logits.size(-1)),
                yb.reshape(-1),
                reduction="none",
            ).reshape(bsz, seq_s)
            for i, ws in enumerate(bws):
                wl = wls[i]
                score_start = 0 if ws == 0 else max(wl - slot_stride, 0)
                sl_loss += slot_nll[i, score_start:wl].to(torch.float64).sum()
                sl_tc += float(wl - score_start)
                tgt = yb[i, score_start:wl]
                prev = xb[i, score_start:wl]
                tb = base_bytes_lut[tgt].to(torch.float64)
                tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                sl_bc += tb.sum()
        if dist.is_available() and dist.is_initialized():
        ...
        slot_val_loss = (sl_loss / sl_tc).item()
        slot_val_bpb = slot_val_loss / math.log(2.0) * (sl_tc.item() / sl_bc.item())
        log0(
            f"final_slot val_loss:{slot_val_loss:.4f} val_bpb:{slot_val_bpb:.4f} "
            f"time:{1000.0 * (time.perf_counter() - t_slot):.0f}ms"
        )
