...
class Hyperparameters:
    ...
    export_calib_seqs = int(os.environ.get("EXPORT_CALIB_SEQS", 16))
    export_temp_grid = tuple(
        float(x)
        for x in os.environ.get("EXPORT_TEMP_GRID", "0.90,0.94,0.98,1.00").split(",")
        if x
    )
    export_proxy_stride = int(os.environ.get("EXPORT_PROXY_STRIDE", 64))
...
def eval_generated_sequences_temperature(
    model: nn.Module,
    ...
    # Calibrate on model-sampled sequences without stitching unrelated samples together.
    my_token_seqs = token_seqs[rank::world_size]
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
    model.eval()
    with torch.inference_mode():
        for batch_start in range(0, len(my_token_seqs), batch_size):
            batch = my_token_seqs[batch_start : batch_start + batch_size]
            x = torch.cat([seq[:, :-1] for seq in batch], dim=0).to(device=device, dtype=torch.int64)
            y = torch.cat([seq[:, 1:] for seq in batch], dim=0).to(device=device, dtype=torch.int64)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = model.forward_logits(x) / temperature
            ...
            if stride is None:
                scored_loss = per_token_loss
                prev = x
                tgt = y
            else:
                scored_loss = per_token_loss[:, -stride:]
                prev = x[:, -stride:]
                tgt = y[:, -stride:]
            loss_sum += scored_loss.to(torch.float64).sum()
            token_count += float(scored_loss.numel())
            token_bytes = base_bytes_lut[tgt.reshape(-1)].to(torch.float64)
            token_bytes += (
                has_leading_space_lut[tgt.reshape(-1)]
                & ~is_boundary_token_lut[prev.reshape(-1)]
            ).to(torch.float64)
...
def eval_val_temperature(
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            token_count += n
            token_bytes = base_bytes_lut[y.reshape(-1)].to(torch.float64)
            token_bytes += (
                has_leading_space_lut[y.reshape(-1)]
                & ~is_boundary_token_lut[x.reshape(-1)]
            ).to(torch.float64)
            byte_count += token_bytes.sum()
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM)
...
def eval_val_sliding_temperature(
    args: Hyperparameters,
    base_model: nn.Module,
    rank: int,
    world_size: int,
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            for i, ws in enumerate(batch_ws):
                ...
                y_batch[i, :wlen] = chunk[1:]
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = base_model.forward_logits(x_batch) / temperature
            nll = F.cross_entropy(
                logits.reshape(-1, logits.size(-1)).float(),
                y_batch.reshape(-1),
                reduction="none",
            ...
            for i, ws in enumerate(batch_ws):
                ...
                scored_nll = nll[i, score_from:wlen].to(torch.float64)
                loss_sum += scored_nll.sum()
...
def find_best_export_temperature(
    args: Hyperparameters,
    model: nn.Module,
    calibration_token_seqs: list[Tensor],
    rank: int,
    world_size: int,
    device: torch.device,
    base_bytes_lut: Tensor,
    ...
    best_temp = 1.0
    best_proxy_bpb = float("inf")
    best_metrics: dict[str, float] = {}
    proxy_stride = args.export_proxy_stride
    use_proxy = 0 < proxy_stride < args.train_seq_len
    temp_grid = args.export_temp_grid or (1.0,)
    for temp in temp_grid:
        block_loss, block_bpb = eval_generated_sequences_temperature(
            model, calibration_token_seqs, rank, world_size, device,
            base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            temperature=temp,
        )
        if use_proxy:
            proxy_loss, proxy_bpb = eval_generated_sequences_temperature(
                model, calibration_token_seqs, rank, world_size, device,
                base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                temperature=temp, stride=proxy_stride,
            )
            proxy_mode = "sliding"
        else:
            proxy_loss, proxy_bpb = block_loss, block_bpb
            proxy_mode = "roundtrip"
        log0(
            f"export_temp_search temp:{temp:.2f} calib_block_bpb:{block_bpb:.8f} "
            f"calib_proxy_bpb:{proxy_bpb:.8f} proxy_mode:{proxy_mode}"
        )
        if proxy_bpb < best_proxy_bpb:
            best_proxy_bpb = proxy_bpb
            best_temp = temp
            best_metrics = {
                ...
                "proxy_bpb": proxy_bpb,
                "proxy_stride": float(proxy_stride),
                "proxy_mode": 1.0 if proxy_mode == "sliding" else 0.0,
            }
...
def main() -> None:
    ...
    export_calib_token_seqs = ar_tokens[: max(1, args.export_calib_seqs)]
    best_temp, export_temp_metrics = find_best_export_temperature(
        args, eval_model, export_calib_token_seqs,
        rank, world_size, device,
        base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        log0,
    )
    proxy_mode = "sliding" if export_temp_metrics.get("proxy_mode", 0.0) > 0.5 else "roundtrip"
    log0(
        f"export_temp_selected temperature:{best_temp:.2f} proxy_mode:{proxy_mode} "
        f"calib_proxy_bpb:{export_temp_metrics['proxy_bpb']:.8f}"
    )
    del ar_tokens
    torch.cuda.synchronize()
    t_qeval = time.perf_counter()
    q_val_loss, q_val_bpb = eval_val_temperature(
    ...
    log0(
        ...
        f"temperature:{best_temp:.2f} eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(
        f"final_int6_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f} "
        f"temperature:{best_temp:.2f}"
    )
    sw_seq_len = effective_eval_seq_len
    candidate_mode = "roundtrip"
    candidate_loss = q_val_loss
    candidate_bpb = q_val_bpb
    candidate_stride = 0
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        torch.cuda.synchronize()
        t_slide = time.perf_counter()
        sw_val_loss, sw_val_bpb = eval_val_sliding_temperature(
        ...
        log0(
            ...
            f"stride:{args.eval_stride} temperature:{best_temp:.2f} "
        ...
        if sw_val_bpb < candidate_bpb:
            candidate_mode = "sliding_window"
            candidate_loss = sw_val_loss
            candidate_bpb = sw_val_bpb
            candidate_stride = args.eval_stride
    if args.eval_stride != 64 and 64 < sw_seq_len:
        torch.cuda.synchronize()
        t_slide64 = time.perf_counter()
        sw64_val_loss, sw64_val_bpb = eval_val_sliding_temperature(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=64,
            temperature=best_temp,
        ...
        log0(
            ...
            f"stride:64 temperature:{best_temp:.2f} eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(
            f"final_int6_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f} "
            f"temperature:{best_temp:.2f}"
        )
        if sw64_val_bpb < candidate_bpb:
            candidate_mode = "sliding_window_s64"
            candidate_loss = sw64_val_loss
            candidate_bpb = sw64_val_bpb
            candidate_stride = 64
    log0(
        f"benchmark_exploit_candidate_exact mode:{candidate_mode} val_loss:{candidate_loss:.8f} "
        f"val_bpb:{candidate_bpb:.8f} temperature:{best_temp:.2f} stride:{candidate_stride}"
    )
    if distributed:
