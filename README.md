...
We recommend starting directly on a remote CUDA machine rather than maintaining a separate local training path.
...
The `train_gpt.py` script is intended as a good launching-off point for new participants, not a SOTA config. We'll accept PRs that tune, improve, or simplify it without significantly increasing complexity, but the best models should stay in the `/records` folder.
