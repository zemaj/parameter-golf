"""
Low-risk competitive experiment fork.

This keeps the cleaner 2026-03-21 11-layer/XSA/EMA/late-QAT structure, but moves the
default hyperparameters closer to the stronger 2026-03-22 run and adds a simple final
evaluation temperature hook for harness-driven calibration sweeps.
"""
...
def main() -> None:
    ...
    log0("candidate:low_risk_11l_ema_sliding_qat_export")
    log0(
        f"final_eval_hook temperature:{args.eval_temperature:.4f} "
        f"mode:{'manual' if args.eval_temperature != 1.0 else 'identity'}"
    )
    ...
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=True) if args.eval_temperature == 1.0 else eval_model
    ...
    log0(
        ...
        f"temperature:{args.eval_temperature:.4f} eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(
        f"final_int6_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f} "
        f"temperature:{args.eval_temperature:.4f}"
    )
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        log0(
            ...
            f"stride:{args.eval_stride} temperature:{args.eval_temperature:.4f} "
    ...
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        log0(
            ...
            f"stride:64 temperature:{args.eval_temperature:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(
